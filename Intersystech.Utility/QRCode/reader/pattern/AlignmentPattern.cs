using System;
using QRCodeDecoder = Intersystech.Utility.QRCode.Codec.QRCodeDecoder;
using Intersystech.Utility.QRCode.Codec.Reader;
using AlignmentPatternNotFoundException = Intersystech.Utility.QRCode.ExceptionHandler.AlignmentPatternNotFoundException;
using InvalidVersionException = Intersystech.Utility.QRCode.ExceptionHandler.InvalidVersionException;
using Intersystech.Utility.QRCode.Geom;
using Intersystech.Utility.QRCode.Codec.Util;

namespace Intersystech.Utility.QRCode.Codec.Reader.Pattern
{
	...
	public class AlignmentPattern
	{
		...
		internal static Point[][] getCenter(bool[][] image, FinderPattern finderPattern, Point[][] logicalCenters)
		{
			...
			canvas.drawCross(centers[0][0], Intersystech.Utility.QRCode.Codec.Util.Color_Fields.BLUE);
			
			axis.Origin = finderPattern.getCenter(FinderPattern.UR);
			centers[sqrtCenters - 1][0] = axis.translate(- 3, 3);
			canvas.drawCross(centers[sqrtCenters - 1][0], Intersystech.Utility.QRCode.Codec.Util.Color_Fields.BLUE);
			
			axis.Origin = finderPattern.getCenter(FinderPattern.DL);
			centers[0][sqrtCenters - 1] = axis.translate(3, - 3);
			canvas.drawCross(centers[0][sqrtCenters - 1], Intersystech.Utility.QRCode.Codec.Util.Color_Fields.BLUE);
			...
			for (int y = 0; y < sqrtCenters; y++)
			{
				for (int x = 0; x < sqrtCenters; x++)
				{
					...
					if (y == 0)
					{
						...
						canvas.drawCross(centers[x][y], Intersystech.Utility.QRCode.Codec.Util.Color_Fields.RED);
					}
					else if (x == 0)
					{
					...
					canvas.drawCross(centers[x][y], Intersystech.Utility.QRCode.Codec.Util.Color_Fields.BLUE);
					canvas.drawLine(new Line(tmpPoint, centers[x][y]), Intersystech.Utility.QRCode.Codec.Util.Color_Fields.LIGHTBLUE);
